...
export default async function ProductDetailPage({ params }: { params: { slug: string } }) {
  ...
  return (
    <main className="min-h-screen bg-[#111] pb-20">
      ...
      {/* ✅ WIDE CONTAINER: Upgraded from max-w-6xl to max-w-[1400px] */}
      <div className="max-w-[1400px] mx-auto p-4 md:p-12">
        ...
        <div className="grid lg:grid-cols-2 gap-16 mb-20 items-start">
          ...
          <div className="relative h-[550px] w-full bg-[#1a1a1a] rounded-[40px] overflow-hidden border border-white/5 flex items-center justify-center group shadow-2xl">
            {product.image && (
          ...
          <div className="flex flex-col pt-4">
            ...
            <h1 className="text-5xl lg:text-7xl font-black text-white mb-8 leading-[0.95] tracking-tighter">
              {product.title}
            </h1>
            ...
            <div className="flex items-center gap-8 mb-12">
              <div className="flex flex-col">
                ...
                <div className="text-7xl font-black text-red-500 tracking-tighter">${currentBestPrice.toFixed(2)}</div>
              </div>
              {hasDiscount && (
                <div className="flex flex-col bg-red-500/10 border border-red-500/20 p-4 rounded-2xl">
        ...
        <div className="bg-[#1a1a1a] rounded-[32px] border border-white/5 overflow-hidden mb-20 shadow-2xl">
          <div className="bg-[#222] px-10 py-6 border-b border-white/5 flex justify-between items-center">
            <h3 className="font-black text-sm uppercase tracking-[0.2em] text-[#888]">Verified Store Comparison</h3>
            <div className="flex items-center gap-2">
                <div className="w-1.5 h-1.5 rounded-full bg-green-500 animate-pulse"></div>
                <span className="text-[10px] font-bold text-[#555] uppercase tracking-widest">Updated Live</span>
            </div>
          </div>
          <div className="divide-y divide-white/5">
            {product.listings.map((listing) => (
              <div key={listing.id} className="flex items-center justify-between p-8 hover:bg-white/[0.02] transition-colors">
                <div className="flex items-center gap-6">
                  <div className="w-14 h-14 relative flex-shrink-0 bg-white rounded-xl flex items-center justify-center overflow-hidden border border-white/10">
                  ...
                  <div className="flex flex-col">
                      <span className="font-black text-white uppercase text-lg tracking-tight">{listing.retailer.name}</span>
                ...
                <div className="flex items-center gap-10">
                  <span className="font-black text-3xl text-white tracking-tighter">${listing.price.toFixed(2)}</span>
                  <TrackedLink 
                    url={listing.url}
                    productId={product.id}
                    retailerId={listing.retailerId}
                    className="bg-primary text-white px-8 py-4 rounded-full text-xs font-black uppercase tracking-widest hover:opacity-90 border border-transparent hover:border-primary/50 transition-all shadow-xl shadow-primary/20"
                  >
                    Go to Store
                  </TrackedLink>
                </div>
        ...
        <div className="mb-20">
          ...
          <div className="bg-[#1a1a1a] p-10 rounded-[40px] border border-white/5 h-[450px] shadow-2xl">
