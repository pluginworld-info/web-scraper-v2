...
export async function POST(req: Request) {
  ...
  try {
    ...
    // Everything below here is YOUR EXACT CODE, just wrapped in a function 
    // so it doesn't force the browser to wait and timeout.
    // =====================================================================
    const runBackgroundSync = async () => {
      try {
        ...
        if (isAborted) {
            // CHANGED: We just return to stop the function. The UI already knows it aborted.
            return; 
        }
        ...
        await prisma.feed.update({ 
         ...
         console.error("Critical Background Sync Error:", bgError);
         await prisma.feed.update({
             where: { id: feedId as string },
             data: { status: 'ERROR', errorMessage: bgError.message }
         });
      }
    };
    ...
    // ⚡ INSTANT RESPONSE: We answer the browser BEFORE the heavy loop finishes!
    return NextResponse.json({ success: true, message: "Sync process running in background" });
