...
export default function AIRecommendations({ initialProducts }: AIRecommendationsProps) {
  ...
  return (
    // ✅ DYNAMIC: Updated border and gradient background to use Primary
    <section className="bg-gradient-to-br from-primary/20 via-[#1a1a1a] to-primary/5 rounded-[40px] p-1 shadow-2xl overflow-hidden relative border border-primary/20">
      
      {/* ✅ DYNAMIC: Animated Glow uses Primary Variable */}
      ...
      <div className="bg-[#1a1a1a]/90 backdrop-blur-3xl rounded-[38px] p-8 md:p-10 relative z-10 h-full">
        
        {/* --- HEADER WITH AI ANIMATION --- */}
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-10">
          <div className="flex items-center gap-5">
            {/* ✅ DYNAMIC: Icon container uses Primary */}
            <div className={`p-4 rounded-2xl border border-primary/20 transition-all duration-700 ${loading ? 'bg-primary/20 shadow-[0_0_25px_rgba(var(--primary-rgb),0.4)] scale-110' : 'bg-primary/10'}`}>
              <svg className={`w-8 h-8 text-primary ${loading ? 'animate-spin' : ''}`} fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            <div>
              <h2 className="text-2xl font-black uppercase tracking-tighter text-white flex items-center gap-3">
                ...
                {isPersonalized && !loading && (
                    // ✅ DYNAMIC: Personalized badge uses Accent
          ...
          <button 
            ...
            disabled={loading}
            // ✅ DYNAMIC: Refresh button uses Primary
            className="text-[10px] uppercase font-black tracking-[0.2em] text-primary hover:text-white flex items-center gap-2 opacity-60 hover:opacity-100 transition-all disabled:opacity-20 group"
          >
            <svg className={`w-4 h-4 group-hover:rotate-180 transition-transform duration-700 ${loading ? 'animate-spin' : ''}`} fill="none" viewBox="0 0 24 24" stroke="currentColor">
          ...
          {loading ? (
             ...
             products.map((product) => (
              <Link 
                ...
                // ✅ DYNAMIC: Card hover border uses Primary
                className="group relative bg-[#111] hover:bg-[#151515] border border-white/5 hover:border-primary/40 rounded-[32px] p-6 transition-all duration-500 flex flex-col hover:-translate-y-2 hover:shadow-2xl hover:shadow-primary/10"
              >
                {/* Match Score Badge */}
                <div className="absolute top-4 right-4 z-10 opacity-0 group-hover:opacity-100 transition-all duration-500 translate-y-2 group-hover:translate-y-0">
                    {/* ✅ DYNAMIC: Match badge uses Primary */}
                ...
                <div className="relative h-36 w-full mb-5 rounded-2xl overflow-hidden bg-[#0a0a0a] p-4 border border-white/5">
                  {product.image ? (
                    <Image 
                      src={product.image} 
                      alt={product.title} 
                      fill 
                      unoptimized 
                      className="object-contain group-hover:scale-110 transition duration-700 grayscale group-hover:grayscale-0 opacity-60 group-hover:opacity-100" 
                    />
                  ) : (
                ...
                <h3 className="font-black text-white text-xs truncate mb-3 group-hover:text-primary transition-colors uppercase tracking-tight">
                  {product.title}
                </h3>
                ...
                <div className="mt-auto flex justify-between items-end">
                  ...
                  {/* ✅ DYNAMIC: Action circle uses Primary */}
                  <div className="w-8 h-8 rounded-full bg-primary/10 flex items-center justify-center group-hover:bg-primary group-hover:text-white transition-all duration-300 text-primary shadow-lg shadow-primary/5 group-hover:shadow-primary/20">
