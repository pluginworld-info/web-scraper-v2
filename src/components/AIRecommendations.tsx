...
export default function AIRecommendations({ initialProducts }: AIRecommendationsProps) {
  ...
  // 1. SIMULATE AI ANALYSIS ON MOUNT
  useEffect(() => {
    ...
    // Trigger analysis if we have ANY user data
    if (history || lastSearch || wishlist) {
      setLoading(true);
      setInsight("Cross-referencing your wishlist and browsing history...");

      // Simulate API delay for "AI Thinking" effect
      setTimeout(() => {
        // --- LOGIC: DETERMINE INSIGHT TEXT ---
        let newInsight = "Selected based on your interest in Mixing & Mastering"; // Fallback
        
        // Priority 1: Wishlist (Strongest Signal)
      ...
      <div className="bg-[#0f0f16]/80 backdrop-blur-xl rounded-xl p-6 md:p-8 relative z-10 h-full">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
          <div className="flex items-center gap-4">
            <div className={`p-3 rounded-xl border border-indigo-500/30 ${loading ? 'bg-indigo-500/20 animate-pulse' : 'bg-indigo-500/10'}`}>
              {/* Custom AI Chip Icon */}
            ...
            <div>
              <h2 className="text-xl font-black uppercase tracking-widest text-white flex items-center gap-2">
                AI Smart Picks
                {isPersonalized && <span className="text-[9px] bg-indigo-500 text-white px-2 py-0.5 rounded-full shadow-lg shadow-indigo-500/50">Personalized</span>}
              </h2>
              ...
              <p className="text-indigo-300 text-xs font-medium tracking-wide flex items-center gap-2 min-h-[1.5em]">
                 {loading ? (
                   <span className="animate-pulse">Processing wishlist & preferences...</span>
                 ) : (
                   insight
                 )}
              </p>
          ...
          {/* Refresh/Train Button (Visual Only for UX) */}
          <button 
            disabled={loading}
            className="text-[10px] uppercase font-bold text-indigo-400 hover:text-white flex items-center gap-1 opacity-50 hover:opacity-100 transition-opacity"
          >
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          {loading ? (
             ...
             products.map((product) => (
              ...
              >
                {/* Match Score Badge (Visual Enhancement) */}
