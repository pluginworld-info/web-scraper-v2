...
export default function AIRecommendations({ initialProducts }: AIRecommendationsProps) {
  ...
  return (
    <section className="bg-gradient-to-br from-primary/20 via-[#1a1a1a] to-primary/5 rounded-[40px] p-1 shadow-2xl overflow-hidden relative border border-primary/20 transition-all duration-500 ease-out">
      ...
      <div className="bg-[#1a1a1a]/90 backdrop-blur-3xl rounded-[38px] p-8 md:p-10 relative z-10 h-full transition-all duration-500 ease-out">
        
        {/* --- HEADER --- */}
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-10">
          <div className="flex items-center gap-5">
            <div className={`p-4 rounded-2xl border border-primary/20 transition-all duration-700 ease-out ${loading ? 'bg-primary/20 shadow-[0_0_25px_rgba(var(--primary-rgb),0.4)] scale-110' : 'bg-primary/10'}`}>
          ...
          <button 
            ...
            disabled={loading}
            className="text-[10px] uppercase font-black tracking-[0.2em] text-primary hover:text-white flex items-center gap-2 opacity-60 hover:opacity-100 transition-all duration-300 ease-out disabled:opacity-20 group"
          >
          ...
          {loading ? (
             ...
             products.map((product) => (
              ...
              >
                ...
                <div className="relative h-36 w-full mb-5 rounded-2xl overflow-hidden bg-[#0a0a0a] p-4 border border-white/5 transition-all duration-500 ease-out">
                  {product.image ? (
                    <Image 
                      src={product.image} 
                      alt={product.title} 
                ...
                <h3 className="font-black text-white text-xs truncate mb-3 group-hover:text-primary transition-colors duration-300 ease-out uppercase tracking-tight">
                  {product.title}
                </h3>
                ...
                <div className="mt-auto flex justify-between items-end">
                  ...
                  <div className="w-8 h-8 rounded-full bg-primary/10 flex items-center justify-center group-hover:bg-primary group-hover:text-white transition-all duration-500 ease-out text-primary shadow-lg shadow-primary/5 group-hover:shadow-primary/20">
