...
interface ProductCardProps {
  product: any; // Using any for flexibility during migration, strictly type this later
  onClick?: (id: string) => void;
}

export default function ProductCard({ product, onClick }: ProductCardProps) {
  // Logic: Lowest Price & Hot Badges
  // Note: This assumes product.listings includes history if we want strict "Lowest Ever" check.
  // For now, we simulate logic based on available props.
    ...
    <div className="group relative bg-[#1e1e1e] rounded-2xl overflow-hidden shadow-xl hover:shadow-2xl transition-all duration-300 border border-gray-800 flex flex-col h-full">
      ...
      <div className="relative aspect-square w-full overflow-hidden bg-white p-4">
        ...
        <div className="absolute top-12 left-3 flex flex-col gap-2 z-10 pointer-events-none">
           ...
           {/* LOWEST PRICE BADGE (Yellow Capsule) */}
           {/* Logic: Only show if discount is massive (>70%) for now to simulate "Lowest Price" logic effectively */}
           {discount > 70 && (
