  ...
  return (
    <div className="bg-[#222222] rounded-2xl shadow-sm border border-[#333] p-8 mb-16">
      <div className="flex items-center justify-between mb-8">
         <h3 className="font-black text-lg uppercase tracking-widest text-[#aaaaaa]">User Reviews</h3>
         <button className="text-sm font-bold text-blue-500 hover:text-white transition-colors">Write a Review</button>
      </div>
          ...
          <div key={review.id} className="border-b border-[#333] last:border-0 pb-8 last:pb-0">
            <div className="flex items-center justify-between mb-2">
               <div className="flex items-center gap-2">
                 <div className="w-8 h-8 rounded-full bg-[#333] flex items-center justify-center font-bold text-xs text-white">
                    {review.user ? review.user.charAt(0) : 'U'}
                 </div>
                 <span className="font-bold text-sm text-white">{review.user || "Anonymous"}</span>
               </div>
               <span className="text-xs text-[#666] font-medium">{review.date || "Recently"}</span>
            </div>
            <div className="flex text-yellow-500 mb-2">
               {[...Array(5)].map((_, i) => (
                  <svg key={i} className={`w-4 h-4 ${i < review.rating ? 'fill-current' : 'text-[#444] fill-current'}`} viewBox="0 0 20 20"><path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" /></svg>
               ))}
            </div>
      ...
      {/* Write Review Form */}
      <div className="bg-[#2a2a2a] rounded-xl p-6 border border-[#333]">
         <h4 className="font-bold text-sm text-white mb-4">Add your rating</h4>
         ...
         ></textarea>
         <button className="bg-black text-white border border-[#333] px-6 py-2 rounded-lg text-sm font-bold uppercase hover:bg-[#111] transition">
           Submit Review
         </button>
      </div>
    </div>
